...
# ---------------------------
    ...
    self._compute_allowed_files(plan, context_pack)
)
                ...
                if not ok_scope:
                    ...
                    for p in oos_paths:
                        ...
                        if (p in found_set) or (self.work_dir / p).exists():
                            candidates.append(p)

                    candidates = list(dict.fromkeys(candidates))
                    if candidates:
                        scope_expanded_files = list(dict.fromkeys(scope_expanded_files + candidates))
                        allowed_union = list(dict.fromkeys(global_plan_files + (files_in_step or []) + scope_expanded_files))
    ...
    self._compute_allowed_files(step_plan, context_pack)
                ...
                if self.cfg.verify_each_step:
                    ...
                    if not verify_ok:
                        ...
                        extra_files = self._extract_compiler_error_files(verify_logs)
                        if extra_files:
